...
namespace OpenDoc.Controllers
{
    public class ExcelController
    {
        ...
        public void CreateExcel(Viaje viaje, Usuario user, string path, int type, string title)
        {
            using (SLDocument doc = new SLDocument())
            {
                ...
                if (type == 1)
                {
                    name_field = user.region == "VELAG" ? "Peso Neto" : "Peso Bruto";
                }
                else
                {
                    name_field = "Peso Bruto (KGS)";
                }
        ...
        private DataTable CreateDatatableList(int ID)
        {
            ...
            string connect = ConfigurationManager.ConnectionStrings["OpenDocConnectionString"].ConnectionString;
            string query = @"select
                ...
                C.peso_bruto as 'Peso Bruto',
	            C.peso_neto as 'Peso Neto', 
	            C.num_paq as Bultos,
