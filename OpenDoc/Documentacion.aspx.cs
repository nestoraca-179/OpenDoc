    ...
    public partial class Documentacion : System.Web.UI.Page
    {
        ...
        protected void GV_GridResultsV_RowCommand(object sender, ASPxGridViewRowCommandEventArgs e)
        {
            ...
            else if (e.CommandArgs.CommandName == "Lista")
            {
                ...
                try
                {
                    new ExcelController().CreateExcel(viaje, user, path, 1, "LISTADO DE DESCARGA");
            ...
            else if (e.CommandArgs.CommandName == "Neptuno")
            {
                ...
                try
                {
                    new ExcelController().CreateExcel(viaje, user, path, 2, "LISTADO NEPTUNO");
        ...
        protected void BTN_GenerarXML_Click(object sender, EventArgs e)
        {
            ...
            if (((Usuario)Session["USER"]).region == "VELAG")
            {
                filename = XMLController.GenerarXMLVELAG(folder, IDSelected); // LA GUAIRA
            }
            else
            {
                filename = XMLController.GenerarXMLVEPBL(folder, IDSelected); // OTROS
            }
